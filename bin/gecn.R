...
#load ("all.CNState.RData")
 ...
 # CNState$Probe.Set.ID <- NULL

  #dim (CNState)
 # CNState <- CNState[,grepl("_Somatic", colnames (CNState))]

  #colnames (CNState)

  #dim (CNState)

 # colnames (CNState) <- gsub ("_Somatic", "", colnames (CNState))
 # cn.data.som <- CNState
#rm(CNState)
...
###rm(snp.data.totxvi.som)
 #load ("CNState.RData")
  ...
  #totxvi.key <- read.csv("20111103_SOM_GERM_SNP6_TOTXVI.csv", as.is=TRUE, stringsAsFactors=FALSE)
 # totxvi.key <- subset (totxvi.key, totxvi.key$Tissue_type == "Somatic")
  #rownames (totxvi.key) <- totxvi.key$Sample

  #colnames (CNState) <- gsub ("-snp6.CN5.CNCHP.txt", "", colnames (CNState))

  #pt.int <- intersect (colnames (CNState), rownames (totxvi.key))

 # CNState <- CNState[,pt.int]

 # totxvi.key <- totxvi.key[pt.int,]

  #colnames (CNState) <- totxvi.key$mrn
